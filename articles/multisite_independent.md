...
delineated in the `site` column. You will also need to edit the
`output_function` column to reflect that this table now should be
considered a Multi Site, Exploratory output.
...
exploratory analysis. Then, the `compute_dist_anomalies` and
`detect_outliers` functions, both available through the `squba.gen`
package, should be executed against your results. You will also need to
edit the `output_function` column to reflect that this table now should
be considered a Multi Site, Anomaly Detection output. Copy the code
below, inputting the table you generated.

`var_col` should be whichever numerical column produced by the
exploratory output that should be used as the target for anomaly
